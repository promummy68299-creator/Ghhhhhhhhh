# bot.py - COMPLETE WORKING HOSTING BOT WITH PYTHON, NODE.JS, ZIP SUPPORT
import os
import sys
import time
import json
...
# =========================
# BOT MANAGER - FIXED FOR PROPER BOT RUNNING
# =========================
class BotManager:
    def __init__(self):
        self.processes = {}
        ...
        try:
            ...
            if bot_type == 'python':
                ...
                if os.path.exists(req_file):
                    logger.info(f"Installing Python dependencies for {bot_name}")
                    subprocess.run(
                ...
                if os.path.exists(pkg_file):
                    logger.info(f"Installing Node.js dependencies for {bot_name}")
                    subprocess.run(
                        ['npm', 'install'],
                        cwd=folder_path,
                        capture_output=True,
            ...
            env = os.environ.copy()
            env['PYTHONUNBUFFERED'] = '1'  # For Python logs
            
            if bot_type == 'python':
                logger.info(f"Starting Python bot: {file_path}")
                ...
                logger.info(f"Starting Node.js bot: {file_path}")
                process = subprocess.Popen(
                    ['node', file_path],
                    cwd=folder_path,
                    stdout=subprocess.PIPE,
            ...
            if process.poll() is not None:
                ...
                return False, f"Bot exited immediately. Check your bot code.\n\nOutput: {stdout[:200]}"
                ...
                if line:
                    ...
                    if line:
                        db_query(
                            ...
                            (0, bot_id, 'stdout', line, datetime.now().isoformat()),
                            commit=True
                        )
                        logger.info(f"Bot {bot_id} log: {line[:100]}")
        except Exception as e:
