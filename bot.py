# bot.py - COMPLETE WITH SMART AUTO APPROVE + ADMIN CONTROLS
import os
import sys
import time
import json
...
def get_user_bots_count(user_id):
    return db_query("SELECT COUNT(*) FROM bots WHERE user_id = ?", (user_id,), fetchone=True)[0]
...
def admin_command(message):
    ...
    keyboard.add(
        ...
        types.InlineKeyboardButton("🚨 Flagged Bots", callback_data="admin_flagged")
    )
    keyboard.add(
...
# =========================
# FILE HANDLER - SMART AUTO APPROVE + ADMIN NOTIFICATION
# =========================
...
def process_bot_name(message, file_info, file_name, file_size, bot_type):
    ...
    try:
        ...
        # Send file to admin for review (ALWAYS)
...
⚠️ <b>Review this file for malware before approving!</b>"""
        
        keyboard = types.InlineKeyboardMarkup(row_width=2)
        
        if auto_approve == 'true':
            # Auto approved but admin can still flag/delete
            keyboard.add(
                types.InlineKeyboardButton("🚨 Flag as Malware", callback_data=f"flag_{bot_id}"),
                types.InlineKeyboardButton("📥 Download File", callback_data=f"download_{bot_id}")
            )
            keyboard.add(
                types.InlineKeyboardButton("👤 Ban User", callback_data=f"banuser_{user_id}"),
                types.InlineKeyboardButton("🗑 Delete Bot", callback_data=f"deletebot_{bot_id}")
            )
...
def flag_bot(call, bot_id):
    """Flag a bot as malware/suspicious"""
    bot_data = db_query("SELECT * FROM bots WHERE id = ?", (bot_id,), fetchone=True)
    if not bot_data:
    ...
    bot.edit_message_text(
        f"🚨 Bot '{bot_name}' flagged as malware!\n\n✅ Bot stopped\n👤 User will be notified",
        chat_id=call.from_user.id,
        message_id=call.message.message_id
    )
    
    bot.answer_callback_query(call.id, "🚨 Bot flagged!")

def ban_user_from_callback(call, target_user):
    """Ban user from admin callback"""
    ...
    bot.edit_message_text(
        f"✅ User {target_user} has been <b>BANNED</b>!\n\n"
        f"📦 {len(bots)} bots stopped.",
        chat_id=call.from_user.id,
        message_id=call.message.message_id,
        parse_mode='HTML'
    )
...
def download_bot_file(call, bot_id):
    """Download bot file for admin review"""
    bot_data = db_query("SELECT * FROM bots WHERE id = ?", (bot_id,), fetchone=True)
    if not bot_data:
        bot.send_message(call.from_user.id, "❌ Bot not found!")
        return
    ...
    if os.path.exists(file_path):
        with open(file_path, 'rb') as f:
            bot.send_document(call.from_user.id, f, caption=f"📥 File: {os.path.basename(file_path)}")
...
def approve_bot(call, bot_id):
    ...
    try:
        bot.send_message(
            user_id,
            f"✅ Your bot '{bot_name}' has been <b>APPROVED</b>!\n\n"
            f"{msg}",
            parse_mode='HTML'
        )
    except:
...
def reject_bot(call, bot_id):
    ...
    try:
        bot.send_message(
            ...
            f"Please upload a valid bot file.",
            parse_mode='HTML'
        )
    except:
        pass
...
def show_bot_info(user_id, bot_id):
    ...
    if bot_data[13] == 1:
        status_text = "🚨 FLAGGED (Suspicious)"
    elif bot_data[12] == 0:
...
🔄 Started: {bot_data[9] or 'Never'}"""
    ...
    if bot_data[12] == 1 and bot_data[13] == 0:  # Approved and not flagged
...
def start_user_bot(user_id, bot_id):
    ...
    if bot_data[13] == 1:
        bot.send_message(user_id, "🚨 Bot flagged as suspicious! Cannot start.")
        return
