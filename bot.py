# bot.py - COMPLETE FIXED HOSTING BOT WITH FULL LOGS
import os
import sys
import time
import json
...
# =========================
# BOT MANAGER - FIXED WITH FULL LOGS
# =========================
class BotManager:
    def __init__(self):
        self.processes = {}
    ...
    def start_bot(self, bot_id):
        try:
            ...
            # ========== START BOT WITH FULL LOG CAPTURE ==========
            env = os.environ.copy()
            ...
            # Start process with stdout/stderr redirect
            if bot_type == 'python':
            ...
            try:
                log_file = os.path.join(bot_data[5], f"{bot_data[2]}_log.txt") if bot_data else "logs/error.txt"
                with open(log_file, 'a') as f:
                    f.write(f"\n=== EXCEPTION ===\n{error_trace}\n")
            except:
                pass
...
# =========================
# SHOW HOSTING LOGS - ADMIN
# =========================
def show_hosting_logs(user_id):
    try:
        # Get hosting bot logs
        log_file = "bot.log"
        ...
        # Also send recent errors from database
        errors = db_query(
