...
def get_user_storage_limit(user_id):
    ...
    return user[7] if user else 1073741824  # 1GB default
...
class BotManager:
    ...
    def monitor_bots(self):
        while self.running:
            try:
                bots = db_query("SELECT * FROM bots WHERE status = 'running'", fetchall=True)
...
def admin_command(message):
    ...
    bot.send_message(user_id, "🔧 <b>Admin Panel</b>\n\nSelect an option:", reply_markup=keyboard, parse_mode='HTML')
...
def handle_document(message):
    ...
    # Check if file is a zip, py, or js
...
def process_bot_name(message, file_info, file_name, file_size, bot_type):
    ...
    try:
        ...
        db_query(
            """INSERT INTO bots 
            (user_id, bot_name, bot_type, file_path, folder_path, status, created_at) 
            VALUES (?, ?, ?, ?, ?, ?, ?)""",
            (user_id, bot_name, bot_type, file_path, bot_folder, 'stopped', datetime.now().isoformat()),
            commit=True
        )
...
def callback_handler(call):
    ...
    if data == "upload":
        ...
        if get_hosting_status() == 'offline':
            bot.send_message(user_id, "🔴 Hosting is currently offline. Please try again later.")
            return
        bot.send_message(user_id, "📤 Please upload your bot file.\n\nSupported formats:\n• .zip (with Python/Node.js files)\n• .py (single Python file)\n• .js (single Node.js file)")
...
def show_bot_info(user_id, bot_id):
    ...
    bot_id, _, bot_name, bot_type, file_path, folder_path, process_id, status, port, created_at, last_started, memory, cpu = bot_data
...
def show_speed(user_id):
    # Get bot performance
    bots = db_query("SELECT id, bot_name, status, memory_usage, cpu_usage FROM bots WHERE user_id = ?", (user_id,), fetchall=True)
