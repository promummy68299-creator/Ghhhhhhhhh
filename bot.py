# bot.py - COMPLETE PRODUCTION READY HOSTING BOT WITH REPLY KEYBOARD
import os
import sys
import time
import json
...
# CONFIGURATION
# =========================
BOT_TOKEN = "8510272747:AAGITizNRgtU1U1ztDGqobZpwEw6NhmNsWU"
ADMIN_ID = 7924753922
...
def admin_keyboard():
    ...
    keyboard.add(types.KeyboardButton("🔙 Back to Main"))
    return keyboard
        ...
        try:
            ...
            if bot_type == 'python':
                ...
                if os.path.exists(req_file):
                    subprocess.run([sys.executable, '-m', 'pip', 'install', '-r', req_file], capture_output=True, text=True)
            elif bot_type == 'node':
            ...
            if bot_type == 'python':
                process = subprocess.Popen([sys.executable, file_path], cwd=folder_path, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            elif bot_type == 'node':
                process = subprocess.Popen(['node', file_path], cwd=folder_path, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            else:
                return False, f"Unsupported bot type: {bot_type}"
            db_query("UPDATE bots SET process_id = ?, status = 'running', last_started = ? WHERE id = ?", (process.pid, datetime.now().isoformat(), bot_id), commit=True)
    ...
    def capture_logs(self, bot_id, process):
        try:
            while process.poll() is None:
                ...
                if line:
                    db_query("INSERT INTO logs (user_id, bot_id, log_type, message, timestamp) VALUES (?, ?, ?, ?, ?)", (0, bot_id, 'stdout', line.strip(), datetime.now().isoformat()), commit=True)
                line = process.stderr.readline()
                if line:
                    db_query("INSERT INTO logs (user_id, bot_id, log_type, message, timestamp) VALUES (?, ?, ?, ?, ?)", (0, bot_id, 'stderr', line.strip(), datetime.now().isoformat()), commit=True)
        except Exception as e:
        ...
        try:
            ...
            if process_id:
                try:
                    ...
                    pass
            db_query("UPDATE bots SET process_id = ?, status = 'stopped' WHERE id = ?", (None, bot_id), commit=True)
...
def process_bot_name(message, file_info, file_name, file_size, bot_type):
    ...
    try:
        ...
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("INSERT INTO bots (user_id, bot_name, bot_type, file_path, folder_path, status, created_at, is_approved, is_flagged) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", (user_id, bot_name, bot_type, file_path, bot_folder, 'stopped', datetime.now().isoformat(), is_approved, 0))
        bot_id = c.lastrowid
        conn.commit()
...
⚠️ <b>Review this file!</b>"""
        ...
        if auto_approve == 'true':
            ...
            keyboard.add(types.InlineKeyboardButton("🚨 Flag", callback_data=f"flag_{bot_id}"), types.InlineKeyboardButton("📥 Download", callback_data=f"download_{bot_id}"))
            keyboard.add(types.InlineKeyboardButton("👤 Ban User", callback_data=f"banuser_{user_id}"), types.InlineKeyboardButton("🗑 Delete", callback_data=f"deletebot_{bot_id}"))
            try:
            ...
            db_query("INSERT INTO pending_approvals (user_id, bot_id, file_name, file_size, uploaded_at) VALUES (?, ?, ?, ?, ?)", (user_id, bot_id, file_name, file_size, datetime.now().isoformat()), commit=True)
            keyboard.add(types.InlineKeyboardButton("✅ Approve", callback_data=f"approve_{bot_id}"), types.InlineKeyboardButton("❌ Reject", callback_data=f"reject_{bot_id}"))
            keyboard.add(types.InlineKeyboardButton("📥 Download", callback_data=f"download_{bot_id}"), types.InlineKeyboardButton("🚨 Flag", callback_data=f"flag_{bot_id}"))
            try:
                with open(file_path, 'rb') as f:
                    bot.send_document(ADMIN_ID, f, caption=admin_msg, reply_markup=keyboard)
            except:
...
🔄 Started: {bot_data[9] or 'Never'}"""
    ...
    if bot_data[12] == 1 and bot_data[13] == 0:
        if bot_data[7] == "running":
            keyboard.add(types.InlineKeyboardButton("⏹ Stop", callback_data=f"bot_{bot_id}_stop"), types.InlineKeyboardButton("🔄 Restart", callback_data=f"bot_{bot_id}_restart"))
        else:
            keyboard.add(types.InlineKeyboardButton("▶️ Start", callback_data=f"bot_{bot_id}_start"))
    keyboard.add(types.InlineKeyboardButton("📋 Logs", callback_data=f"bot_{bot_id}_logs"), types.InlineKeyboardButton("🗑 Delete", callback_data=f"bot_{bot_id}_delete"))
...
• Live logs
• File manager
📞 Support: @pro_tg01""", parse_mode='HTML')
