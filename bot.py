# bot.py - COMPLETE FIXED HOSTING BOT
import os
import sys
import time
import json
...
class BotManager:
    ...
    def start_bot(self, bot_id):
        try:
            ...
            threading.Thread(target=capture_with_file, daemon=True).start()
            ...
            if process.poll() is not None:
                ...
                db_query(
                    "INSERT INTO logs (user_id, bot_id, log_type, message, timestamp) VALUES (?, ?, ?, ?, ?)",
                    (user_id, bot_id, 'error', f"Bot exited: {error_msg[:200]}", datetime.now().isoformat()),
                    commit=True
                )
                ...
                if os.path.exists(log_file):
                    try:
                        with open(log_file, 'rb') as f:
                            bot.send_document(ADMIN_ID, f, caption=f"📋 Logs for {bot_name}")
                    except:
                        pass
...
def handle_document(message):
    ...
    # Ask for bot name using inline keyboard instead of text input
