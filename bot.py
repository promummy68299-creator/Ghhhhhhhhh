...
def api_call(endpoint, method='GET', payload=None, params=None):
    ...
    for attempt in range(3):
        try:
            ...
            logger.warning(f"Attempt {attempt+1} to {endpoint} failed: {e}")
...
def start_cmd(msg):
    ...
    if admin is None:
        ...
        if resp and resp.get('success'):
            ...
            # Even if admin setup fails, we still reply to let the user know the bot is alive
            bot.reply_to(msg, f"👋 Hello {msg.from_user.first_name}!\nBot is working, but admin setup failed. Please try again later.\nSend me a URL to shorten.")
    else:
...
def stats_cmd(msg):
    ...
    if 'error' not in data and 'links' in data:
        ...
        reply += "❌ Could not retrieve your stats (API error)."
...
def handle_url(msg):
    ...
    if result and 'shortcode' in result:
        ...
        bot.reply_to(msg, "❌ Error shortening. Please try again later (API unreachable).")
